...
/**
 * Example Social Media Storm Declarator class (simple example without exchange
 * and routing keys). Will be passed to the main Social Media AMQP Spout and
 * will declare specified RabbitMQ queue. Mainly needed in case if Storm spout
 * would start to receive messages before they would be actually be sent
 *
 * Documentation (no API, just an example of usage):
 * https://github.com/ppat/storm-rabbitmq/blob/master/README.md (search for
 * "Declarator")
 */
public class ExampleSocialMediaStormDeclarator implements Declarator {
	...
	private boolean boolPersistence;

	/**
	 * Main constructor
	 *
	 * @param strExchange
	 *            rabbitmq exchange value
	 * @param strExchangeType
	 *            rabbitmq exhange type (e.g. topic, direct etc)
	 * @param strRoutingKey
	 *            rabbitmq routing key
	 * @param strQueueName
	 *            rabbitmq queue name
	 */
	public ExampleSocialMediaStormDeclarator(String strExchange, String strExchangeType, String strRoutingKey,
			String strQueueName, boolean boolPersistence) {
	...
	public void execute(Channel channel) {
		try {
			 ...
			 */
			// TODO: make the "false" a parameter
