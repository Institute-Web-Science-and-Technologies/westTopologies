...
import uniko.west.westtopology.bolts.LocationTopicModelBolt;
...
public class TopologyRunner {
	...
	LocationTopicModelBolt locationTopicModelBolt;
	...
	try {
	    ...
	    Logger.getLogger(TopologyRunner.class.getName()).log(Level.SEVERE,
		    null, ex);
	}
	...
	// locationCrawlerBolt = new LocationCrawlerBolt(emitFieldsId,
	// restletURL);
	// boltDeclarer = builder.setBolt("locationCrawlerId",
	// locationCrawlerBolt);
	// boltDeclarer.shuffleGrouping(spoutId);
	discussionTreeBolt = new DiscussionTreeBolt(emitFieldsId);
	boltDeclarer = builder.setBolt("discussionTeeBoltId",
		discussionTreeBolt);
	boltDeclarer.shuffleGrouping(spoutId);
	...
	boltDeclarer = builder.setBolt("interactionGraphBoltId",
		interactionGraphBolt);
	boltDeclarer.shuffleGrouping(spoutId);

	locationTopicModelBolt = new LocationTopicModelBolt(emitFieldsId,
		"/home/martin/reveal/files/wordmap.txt");
	...
	try {
	    ...
	    StormSubmitter.submitTopology("westTopology", conf,
	...
	Logger.getLogger(TopologyRunner.class.getName()).log(Level.INFO,
		"Submitted the example Storm topology.");
